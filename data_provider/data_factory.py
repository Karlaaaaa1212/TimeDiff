...
from data_provider.data_loader import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_Custom_S, Dataset_wind, Dataset_Monash, Dataset_Stock
...
def data_provider(args, flag, return_full_data=False, shuffle_flag_train=True):
    ...
    if args.dataset_name in ['ECL','ETTh1','ETTh2','ETTm1','ETTm2','Exchange','traffic','weather','illnes','wind','stock']:
        ...
        if flag in ['test', 'val']:
            shuffle_flag = False
            # never drop the tail when evaluating: the last partial batch is real
            # data, and dropping it silently cuts the end off the test period
            # (with 1238 windows / batch 32 that is the last 22 days of 2024).
            # test() already concatenates instead of stacking, so a short final
            # batch is fine.
            drop_last = False
