...
class CNN_DiffusionUnet(nn.Module):
    ...
    def __init__(self, args, num_vars, seq_len, pred_len, diff_steps):
        ...
        # the extreme-scenario knob rides along as two extra columns of cond_info
        # (value + presence mask, one pair per channel) -> 2*num_vars extra channels
        ...
        if self.args.ablation_study_case  in ["mix_1", "mix_ar_0"]:
            self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+self.num_vars+knob_channels, self.num_vars, num_layers=args.ddpm_layers_II)
        else:
            self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+self.num_vars+self.num_vars+knob_channels, self.num_vars, num_layers=args.ddpm_layers_II)

    def forward(self,  yn=None, diffusion_step=None, cond_info=None, y_clean=None):
        """
        cond_info : [bsz, num_vars, seq_len + pred_len]                  (no knob)
                    [bsz, num_vars, seq_len + pred_len + 2]              (with knob:
                    the last two columns are the per-channel knob value and its mask)
        """
        ...
        if self.use_knob:
            knob_val = cond_info[:, :, -2].unsqueeze(-1).repeat(1, 1, self.pred_len)
            knob_mask = cond_info[:, :, -1].unsqueeze(-1).repeat(1, 1, self.pred_len)
            inp = torch.cat([inp, knob_val, knob_mask], dim=1)
