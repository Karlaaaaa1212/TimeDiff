...
class Diffusion_Worker(nn.Module):
    ...
    def __init__(self, args, u_net=None, diff_steps=1000):
        ...
        # "v" (velocity) is here because this schedule ends at alpha_bar = 2.4e-7.
        #   x_start : the net may ignore x_t entirely and decode cond -> samples collapse
        #   noise   : rebuilding x0 divides by sqrt(alpha_bar) ~ 1/2029 -> samples explode
        #   v       : sqrt(a)*eps - sqrt(1-a)*x0 stays O(1) AND cannot be produced without
        #             reading x_t, so the shortcut is closed and the sampler stays stable
        assert self.parameterization in ["noise", "x_start", "v"], \
            'supported: "noise" (eps), "x_start" (x0), "v" (velocity)'
    ...
    def set_new_noise_schedule(self, given_betas=None, beta_schedule="linear", diff_steps=1000, beta_start=1e-4, beta_end=2e-2
        ...
        if self.parameterization in ["noise", "v"]:
    ...
    def get_v(self, x_start, noise, t):
        ...
        It is the velocity of the probability-flow trajectory at time t.  Both terms are
        bounded by 1, so the target is O(1) at every noise level -- unlike eps, whose
        inverse blows up as alpha_bar -> 0.  Crucially it contains eps, which can only be
        recovered from x_t, so a network that ignores x_t cannot fit this target.
        """
    ...
    def predict_start_from_v(self, x_t, t, v):
        """
        Invert get_v at sampling time.  Substituting x_t = sqrt(a)*x0 + sqrt(1-a)*eps into
        v = sqrt(a)*eps - sqrt(1-a)*x0 and eliminating eps gives
    ...
    def p_mean_variance(self, x, t, cond_ts, clip_denoised: bool):
        ...
        if self.parameterization == "noise":
            ...
            model_out = self.predict_start_from_v(x, t=t, v=model_out)
